...
export default function LiveScoring() {
  ...
  // Auto-load game when arriving via Game Day button from Schedule
  useEffect(() => {
            ...
            {[...games].filter(g => g.result && g.date).sort((a, b) => b.date.localeCompare(a.date)).map(g => {
              ...
              return (
                <div key={g.id} className="card" style={{
                  marginBottom: '10px', background: 'var(--gray-50)', border: '1px solid var(--gray-200)'
                }}>
                  <div style={{ display: 'flex', gap: '12px', alignItems: 'flex-start' }}>
                    <div style={{
                      background: g.result === 'W' ? '#DCFCE7' : '#FEE2E2',
                      color: g.result === 'W' ? '#16A34A' : '#B91C1C',
                      ...
                      <div style={{ fontSize: '14px', fontWeight: '700', fontFamily: 'Oswald, sans-serif' }}>{g.result}</div>
                    </div>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
                        <span style={{ fontWeight: '700', fontSize: '16px' }}>vs {g.opponent}</span>
                        {g.score && (
                          <span style={{ fontFamily: 'Oswald, sans-serif', fontWeight: '700', fontSize: '15px', color: 'var(--gray-700)' }}>
                            {typeof g.score === 'object' ? `${g.score.us}–${g.score.them}` : g.score}
                          </span>
                        )}
                      </div>
                      {g.location && <div style={{ fontSize: '12px', color: 'var(--gray-400)', marginTop: '1px' }}>📍 {g.location}</div>}
