...
export default function Stats() {
  ...
  const saveFielding = async (playerId) => {
    ...
    await setDoc(ref, {
      ...current,
      fielding: { ...existing, [editFieldingPos]: {
        innings: parseInt(editFieldingStats.innings) || 0,
        putouts: parseInt(editFieldingStats.putouts) || 0,
        assists: parseInt(editFieldingStats.assists) || 0,
        errors:  parseInt(editFieldingStats.errors)  || 0,
      }}
    }, { merge: true });
                  ...
                  <tr key={player.id} style={{ borderBottom: '1px solid var(--gray-100)', background: i % 2 === 0 ? 'white' : 'var(--gray-50)' }}>
                    ...
                    {canEdit && (
                      <td style={{ padding: '8px 6px', textAlign: 'center' }}>
                        <button onClick={() => { setEditingPlayer(player.id); setEditSection('fielding'); const pos = FIELDING_POSITIONS[0]; setEditFieldingPos(pos); const ff = allStats[player.id]?.fielding || {}; setEditFieldingStats(ff[pos] || { innings:0,putouts:0,assists:0,errors:0 }); }}
  ...
  const HittingPracticeView = () => {
    ...
    const rows = players
      .map(p => {
        let entry;
        if (practiceDate) {
          entry = allStats[p.id]?.practiceLogs?.[practiceDate];
        } else {
        ...
        {FIELDING_POSITIONS.map(pos => {
          const rows = players
            .map(p => {
              let f;
              if (practiceDate) {
                f = allStats[p.id]?.practiceLogs?.[practiceDate]?.fielding?.[pos];
              } else {
                const logs = Object.values(allStats[p.id]?.practiceLogs || {});
                const combined = logs.reduce((acc, l) => {
                  const lf = l.fielding?.[pos];
