...
export default function LogPracticeModal({ players, practiceSchedule = [], onClose, onSaved }) {
  ...
  const setBat = (id, key, delta) => {
    ...
    setEntries(prev => ({ ...prev, [id]: { ...e, batting: { ...e.batting, [key]: val }, saved: false } }));
  };

  const addPos = (id, pos) => {
    const e = getEntry(id);
    if (e.fieldingList.find(f => f.pos === pos)) return;
    setEntries(prev => ({ ...prev, [id]: { ...e, fieldingList: [...e.fieldingList, { pos, innings: 1, putouts: 0, assists: 0, errors: 0 }], saved: false } }));
  };
